 ...
 *
 */
public abstract class BaseDBInsertableAnvizentFlatMapFunction<T, R, E> implements FlatMapFunction<T, R> {
	...
	public BaseDBInsertableAnvizentFlatMapFunction(ConfigBean configBean, MappingConfigBean mappingConfigBean, LinkedHashMap<String, AnvizentDataType> structure,
	...
	public Iterator<R> call(T row) throws Exception {
		...
		if (row == null) {
			...
			do {
				...
				try {
					BaseDBRetrievalRow<T, Iterator<R>> dbRetrievalRow = process(row);
					Iterator<R> outputRows = dbRetrievalRow.getOutputRow();

					accumulate(StatsCategory.OUT, row, outputRows, dbRetrievalRow, null);
					accumulate(StatsCategory.OTHER, row, outputRows, dbRetrievalRow, null);
	...
	private void accumulate(StatsCategory accumulatorKey, T inputRow, Iterator<R> outputRow, BaseDBRetrievalRow sqlRetrievalRow,
	...
	public abstract BaseDBRetrievalRow<T, Iterator<R>> process(T row) throws RecordProcessingException, ValidationViolationException, DataCorruptedException;

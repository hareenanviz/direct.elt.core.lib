...
public abstract class BaseAnvizentRetrievalFunction<T, R, E> implements FlatMapFunction<T, R> {
	...
	public BaseAnvizentRetrievalFunction(ConfigBean configBean, MappingConfigBean mappingConfigBean, LinkedHashMap<String, AnvizentDataType> structure,
		...
		if (row == null) {
			...
			do {
				...
				try {
					...
					R newRow = (R) sqlRetrievalRow.getOutputRow();
	...
	private void accumulate(StatsCategory accumulatorKey, T inputRow, R outputRow, BaseDBRetrievalRow sqlRetrievalRow, AnvizentFunctionException exception) {
	...
	public abstract BaseDBRetrievalRow<T, R> process(T row) throws ValidationViolationException, RecordProcessingException, DataCorruptedException;
